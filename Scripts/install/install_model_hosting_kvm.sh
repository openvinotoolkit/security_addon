...
chmod 700 /opt/ovsa/kvm/example_runtime/start_secure_ovsa_kvm_model_server.sh 2>&1 | sed 's/^/    /'

echo "Loading the docker image..."
if [[ "$(docker images -q openvino/model_server-ovsa_kvm-nginx-mtls 2> /dev/null)" == "" ]]; then
        echo "Docker does not exist."
else
        echo "Removing existing docker image..."
        docker image rm -f openvino/model_server-ovsa_kvm-nginx-mtls 2>&1 | sed 's/^/    /'
fi

docker load -i model_server-ovsa_kvm-nginx-mtls.tar.gz 2>&1 | sed 's/^/    /'
