...
set -x
mkdir -p $ARTEFACTS_PATH/vd/1
cd $ARTEFACTS_PATH/vd
...
#Key, CSR & Self-signed certificate generation:for vehicle detection 
...
set -x
curl --create-dirs \
     https://download.01.org/opencv/2021/openvinotoolkit/2021.1/open_model_zoo/models_bin/1/vehicle-detection-adas-0002/FP32/vehicle-detection-adas-0002.xml \
     https://download.01.org/opencv/2021/openvinotoolkit/2021.1/open_model_zoo/models_bin/1/vehicle-detection-adas-0002/FP32/vehicle-detection-adas-0002.bin \
     -o $ARTEFACTS_PATH/vehicle-detection-adas-0002.xml -o $ARTEFACTS_PATH/vehicle-detection-adas-0002.bin
set +x
echo ""
...
set -x
uuid=$(uuidgen)
$EXECUTION_PREFIX ./ovsatool controlAccess -i $ARTEFACTS_PATH/vehicle-detection-adas-0002.xml $ARTEFACTS_PATH/vehicle-detection-adas-0002.bin \
                                -n "vehicle detection" -d "vehicle detection retail" -v 0002 \
                                -p $OVSA_DEV_ARTEFACTS/vehicle_detection_model.dat -m $OVSA_DEV_ARTEFACTS/vehicle_detection_model.masterlic -k $KEYSTORE_PATH/isv_keystore \
                                -g $uuid
set +x
echo ""
...
set -x
if [[ $1 == "SGX" ]]; then
	$EXECUTION_PREFIX ./ovsaruntime gen-tcb-signature -n "Vehicle Detect Runtime TCB" -v "1.0" \
                                       -f $OVSA_DEV_ARTEFACTS/vehicle_detect_runtime.tcb -k $KEYSTORE_PATH/isv_keystore \
									   -s $ARTEFACTS_PATH/entrypoint.sig
else
	./ovsaruntime gen-tcb-signature -n "Vehicle Detect @ Runtime VM" -v "1.0" \
                                       -f $OVSA_DEV_ARTEFACTS/vehicle_detect_runtime.tcb -k $KEYSTORE_PATH/isv_keystore
fi
set +x
echo ""
...
# Create Customer KS - for vehicle detection 
...
set -x
#Time limit license config: for face detection 
...
set -x
$EXECUTION_PREFIX ./ovsatool sale -m $OVSA_DEV_ARTEFACTS/vehicle_detection_model.masterlic -k $KEYSTORE_PATH/isv_keystore -l $OVSA_DEV_ARTEFACTS/30daylicense.config -t $OVSA_DEV_ARTEFACTS/vehicle_detect_runtime.tcb -p $ARTEFACTS_PATH/primary_custkeystore.csr.crt -c $OVSA_DEV_ARTEFACTS/vehicle_detection_model.lic
set +x
echo ""
...
set -x
cp $OVSA_DEV_ARTEFACTS/vehicle_detection_model.lic $OVSA_DEV_ARTEFACTS/1/vehicle_detection_model.lic
cp $OVSA_DEV_ARTEFACTS/vehicle_detection_model.dat $OVSA_DEV_ARTEFACTS/1/vehicle_detection_model.dat
...
"model_config_list":[
        ...
        "config":{
                ...
                "custom_loader_options": {"loader_name":  "ovsa", "keystore":  "$KEYSTORE_PATH/custkeystore", "controlled_access_file": "vehicle_detection_model"}
        }
        }
]
}
...
echo "1) $OVSA_DEV_ARTEFACTS/1/vehicle_detection_model.lic"
echo "2) $OVSA_DEV_ARTEFACTS/1/vehicle_detection_model.dat"
echo ""
...
echo "1) $OVSA_DEV_ARTEFACTS/vehicle_detection_model.lic"
