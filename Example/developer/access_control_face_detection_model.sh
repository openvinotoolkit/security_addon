...
set -x
mkdir -p $ARTEFACTS_PATH/fd/1
cd $ARTEFACTS_PATH/fd
...
#Key, CSR & Self-signed certificate generation:for face detection 
...
curl --create-dirs \
     ...
     -o $ARTEFACTS_PATH/face-detection-retail-0004.xml -o $ARTEFACTS_PATH/face-detection-retail-0004.bin
set +x
echo ""
...
set -x
uuid=$(uuidgen)
$EXECUTION_PREFIX ./ovsatool controlAccess -i $ARTEFACTS_PATH/face-detection-retail-0004.xml $ARTEFACTS_PATH/face-detection-retail-0004.bin \
                                -n "face detection" -d "face detection retail" -v 0004 \
                                -p $OVSA_DEV_ARTEFACTS/face_detection_model.dat -m $OVSA_DEV_ARTEFACTS/face_detection_model.masterlic -k $KEYSTORE_PATH/isv_keystore \
                                -g $uuid
set +x
echo ""
...
set -x
if [[ $1 == "SGX" ]]; then
	$EXECUTION_PREFIX ./ovsaruntime gen-tcb-signature -n "Face Detect Runtime TCB" -v "1.0" \
                                       -f $OVSA_DEV_ARTEFACTS/face_detect_runtime.tcb -k $KEYSTORE_PATH/isv_keystore \
									   -s $ARTEFACTS_PATH/entrypoint.sig
else
	./ovsaruntime gen-tcb-signature -n "Face Detect @ Runtime VM" -v "1.0" \
                                       -f $OVSA_DEV_ARTEFACTS/face_detect_runtime.tcb -k $KEYSTORE_PATH/isv_keystore
fi
set +x
echo ""
...
# Create Customer KS - for face detection 
...
set -x
$EXECUTION_PREFIX ./ovsatool sale -m $OVSA_DEV_ARTEFACTS/face_detection_model.masterlic -k $KEYSTORE_PATH/isv_keystore -l $OVSA_DEV_ARTEFACTS/30daylicense.config -t $OVSA_DEV_ARTEFACTS/face_detect_runtime.tcb -p $ARTEFACTS_PATH/primary_custkeystore.csr.crt -c $OVSA_DEV_ARTEFACTS/face_detection_model.lic
set +x
echo ""
...
set -x
cp $OVSA_DEV_ARTEFACTS/face_detection_model.lic $OVSA_DEV_ARTEFACTS/1/face_detection_model.lic
cp $OVSA_DEV_ARTEFACTS/face_detection_model.dat $OVSA_DEV_ARTEFACTS/1/face_detection_model.dat
...
"model_config_list":[
        ...
        "config":{
                ...
                "custom_loader_options": {"loader_name":  "ovsa", "keystore":  "$KEYSTORE_PATH/custkeystore", "controlled_access_file": "face_detection_model"}
        }
        }
]
}
...
echo "1) $OVSA_DEV_ARTEFACTS/1/face_detection_model.lic"
echo "2) $OVSA_DEV_ARTEFACTS/1/face_detection_model.dat"
echo ""
...
echo "1) $OVSA_DEV_ARTEFACTS/face_detection_model.lic"
