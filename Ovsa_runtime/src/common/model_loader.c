...
static ovsa_status_t ovsa_do_decrypt_model_files(const int asym_key_slot, const int peer_slot,
                                                 ovsa_customer_license_sig_t* customer_lic_sig,
                                                 ovsa_protected_model_sig_t* protect_model_sig,
                                                 char** decrypt_xml, char** decrypt_bin,
                                                 int* xml_len, int* bin_len) {
    ...
    if (ret < OVSA_OK) {
        OVSA_DBG(DBG_E, "Error: Unwrap model encryption key failed with code  %d\n", ret);
        goto out;
    }
    ...
    head           = protect_model_sig->protect_model.enc_model;
    enc_model_list = head;

    if (enc_model_list == NULL) {
        OVSA_DBG(DBG_E, "Error: Model file empty  \n");
    } else {
        while (enc_model_list != NULL) {
            ...
            if (ret < OVSA_OK) {
                OVSA_DBG(DBG_E, "Error: Could not get length of additional_param string %d\n", ret);
                goto out;
            }
            /* Decrypt protected model files */
            ...
            if (ret != OVSA_OK || decrypted_model_buf == NULL) {
                OVSA_DBG(DBG_E, "Error: Decrypt protected model files failed with code %d\n", ret);
            ...
            if (*decrypt_xml == NULL && (fnmatch(xml_pattern, enc_model_list->file_name, 0) == 0)) {
                ...
                if (ret < OVSA_OK) {
                    ...
                    OVSA_DBG(DBG_E, "Error: Memory alloc fail for xml file with code %d\n", ret);
                    ovsa_safe_free(&decrypted_model_buf);
                    goto out;
                }
        ...
        OVSA_DBG(DBG_D, "\nProtected Model files Decrypted Successfully \n");
    }
out:
    /* clear key/IV/HMAC from the key slot */
    ovsa_crypto_clear_symmetric_key_slot(keyiv_hmac_slot);
...
ovsa_status_t ovsa_start_model_loader(const int asym_key_slot, const int peer_slot,
                                      ovsa_customer_license_sig_t* customer_lic_sig,
                                      ovsa_protected_model_sig_t* protect_model_sig,
    ...
    ret = ovsa_do_decrypt_model_files(asym_key_slot, peer_slot, customer_lic_sig, protect_model_sig,
                                      decrypt_xml, decrypt_bin, xml_len, bin_len);
    if (ret < OVSA_OK) {
...
out:
    OVSA_DBG(DBG_D, "OVSA:%s Exit\n", __func__);
    ovsa_safe_free_enc_list(&protect_model_sig->protect_model.enc_model);
    return ret;
}
